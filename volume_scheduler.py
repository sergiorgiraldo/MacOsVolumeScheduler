...
class VolumeScheduler:
    ...
    def run(self):
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Volume Scheduler started")
        ...
        try:
            ...
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Volume Scheduler stopped")
            # Clean up PID file
            try:
                if PID_FILE.exists():
                    PID_FILE.unlink()
...
def main():
    if len(sys.argv) > 1:
        ...
        if command == 'start':
            ...
            print("Starting Volume Scheduler in background...")
            print(f"Logs: {CONFIG_DIR / 'scheduler.log'}")
